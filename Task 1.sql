...
-- START HERE
/*Summary of the solution - Creating a new table with the name 'enquiry_txn' to store the stacked user_ids and dates using 'Union All'*/
...
INSERT INTO enquiry_txn(user_id, date, count_enqs, count_txns)
WITH
uc AS( /* Adding count of txns as 0 as they are not present in the enquiries table*/

select user_id as user_id, date as date, count(enquiry_id) as count_enqs, 0 as count_txns from enquiries group by user_id, date

UNION ALL /*Using union to stack the cloumns into rows*/

/* Adding count of enquiries as 0 as they are not present in the txns table*/
select user_id as user_id, date as date, 0 as count_enqs, count(txn_id) as count_txns from txns group by user_id, date
)
select uc.user_id, uc.date, SUM(uc.count_enqs), SUM(uc.count_txns) from uc group by uc.user_id, uc.date order by user_id;

select * from enquiry_txn;
