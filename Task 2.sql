...
--START HERE
--Solution Summary: This query maps txns to their corresponding enquiries within a 30-day window using ROW_NUMBER + PARTITION BY and ensuring a 1:1 relationship using ORDER BY DESC and rn=1. It then aggregates txns IDs for each enquiry.
...
WITH 
  mapped_txns AS (
    ...
    JOIN enquiries e
       ...
       AND e.date <= t.date
       AND t.date <= e.date + 30
),
-- Step 2: Filter to 1:1 txn → enquiry mapping
 ranked_txns AS (
    SELECT txn_id, mapped_txns.date, user_id, enquiry_id
    FROM mapped_txns
    WHERE rn = 1
)
-- Step 3: Group by enquiry and collect txn_ids into array
SELECT 
    e.enquiry_id,
    e.date AS enquiry_date,
    e.user_id,
    ARRAY_AGG(rt.txn_id) AS txn_ids
FROM enquiries e
LEFT JOIN ranked_txns rt ON e.enquiry_id = rt.enquiry_id
